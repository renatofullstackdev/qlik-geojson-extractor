export function buildPropertyDefinitions(selectedProperties = [], customProperties = []) {
  ...
  for (const item of custom) {
    if (!item.label || !item.expression) throw new Error("Toda propriedade por expressão deve possuir rótulo e expressão Qlik.");
  }
...
export function extractionHealth(result) {
  ...
  if (uniqueKeys > 0 && featureCount === 0 && missingCount > 0) {
    return { level: "error", message: `Nenhuma feição foi gerada: ${missingCount} entidade(s) real(is) ficaram sem geometria Point válida. Revise a fonte espacial antes de baixar.`, allowDownload: false, rowCount, featureCount, uniqueKeys, missingCount, skippedNullCount };
  }
  if (missingCount > 0) {
    return { level: "warning", message: `GeoJSON gerado com ${missingCount} entidade(s) sem geometria Point válida. Revise os registros antes de usar o arquivo.`, allowDownload: featureCount > 0, rowCount, featureCount, uniqueKeys, missingCount, skippedNullCount };
  }
...
export function coordinateFieldGroups(fields = [], detectedField = "") {
  const normalized = fields.map((field) => ({ name: String(field?.name ?? ""), cardinality: field?.cardinality ?? null, tags: Array.isArray(field?.tags) ? field.tags : [], sourceTables: field?.sourceTables ?? [] })).filter((field) => field.name);
...
export function locationFieldGroups(fields = [], detectedField = "", referencedFields = []) {
  const normalized = fields.map((field) => ({ name: String(field?.name ?? ""), cardinality: field?.cardinality ?? null, tags: Array.isArray(field?.tags) ? field.tags : [], sourceTables: field?.sourceTables ?? [] })).filter((field) => field.name);
  const byName = new Map(normalized.map((field) => [field.name, field]));
  const detected = detectedField && byName.has(detectedField) ? byName.get(detectedField) : null;
  const referenced = referencedFields.map((name) => byName.get(name)).filter(Boolean).filter((field) => field.name !== detected?.name);
  const geo = normalized.filter((field) => field.name !== detected?.name && !referenced.some((item) => item.name === field.name) && field.tags.some((tag) => ["$geopoint", "$geoname"].includes(tag))).sort((a, b) => a.name.localeCompare(b.name, "pt-BR"));
  const other = normalized.filter((field) => field.name !== detected?.name && !referenced.some((item) => item.name === field.name) && !geo.some((item) => item.name === field.name)).sort((a, b) => a.name.localeCompare(b.name, "pt-BR"));
  return { detected, referenced, geo, other };
}
...
export function relatedFields(fields = [], entityKey, ...spatialFieldNames) {
  const byName = new Map(fields.map((field) => [field.name, field]));
  const anchorTables = new Set();
  for (const name of [entityKey, ...spatialFieldNames].filter(Boolean)) {
...
export function buildDiagnosticReport({ inspectionReport, layerIndex, config, result }) {
  ...
  return {
    ...
    pointLayer: layer ? { objectId: layer.objectId, layerId: layer.layerId, layerIndex: layer.layerIndex, spatialMode: layer.spatialMode } : null,
    spatial: diagnostic ? {
      mode: diagnostic.spatialMode,
      source: diagnostic.spatialSource,
      cardinality: diagnostic.spatialCardinality,
      statistics: diagnostic.spatialStats,
      warnings: diagnostic.warnings
    } : null,
    coordinates: diagnostic?.spatialMode === "coordinates" ? {
    ...
    location: diagnostic?.spatialMode === "location" ? {
      definition: diagnostic.locationDefinition,
      cardinality: diagnostic.locationCardinality,
      statistics: diagnostic.spatialStats,
      warnings: diagnostic.warnings
    } : null,
    ...
    extraction: result ? {
      spatialMode: result.spatialMode ?? config?.spatialMode ?? "coordinates",
      rowCount: result.rowCount ?? 0,
      featureCount: result.featureCount ?? 0,
      uniqueKeys: result.uniqueKeys ?? 0,
      missingGeometry: result.missing?.length ?? 0,
